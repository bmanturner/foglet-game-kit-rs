...
- [x] **5e** — Implement `get_presence(player_id)` returning optional row. Test: unplaced player returns `None`. [5b]
- [x] **5f** — Implement `players_at(place_id)`. Test: returns current occupants and excludes players who have moved on. [5c]
- [x] **5g** — Confirm the kit does not auto-place players. Test: a freshly upserted player has no presence row until `set_presence` is called. [5b, v2 Task 5]

### Task 6 — Place recall

- [x] **6a** — Add `place_recall` migration: `(player_id, place_id)` composite PK, `first_seen_at`, `last_seen_at`, `snapshot_json`. [3a, v2 Task 5]
- [x] **6b** — Implement `touch_recall(player_id, place_id, snapshot_json?)` idempotent upsert. Test: first call inserts; second call updates `last_seen_at`. [6a]
...
- [x] **7d** — Confirm `equilibrium` is advisory. Test: setting `equilibrium` does not alter `quantity`; the kit performs no auto-drift. [7b]
- [x] **7e** — Implement `slots_for_owner(owner_kind, owner_id)`. Test: deterministic ordering. [7b]
- [x] **7f** — Implement explicit `merge_slots(slot_a, slot_b)` helper that merges only when `(owner_kind, owner_id, item_key, metadata_json)` match exactly. Test: mismatched metadata refuses to merge. [7b]
...
- [x] **8b** — Test rejection when source lacks stock; no partial mutation. [8a]
- [x] **8c** — Test rejection of zero or negative quantity. [8a]
- [x] **8d** — Pass post-mutation slot snapshots to `on_commit`. Test: callback observes new quantities for both source and dest. [8a]
- [x] **8e** — Roll back the entire transfer when `on_commit` returns an error. Test: source and dest unchanged after rejection. [8d]
- [x] **8f** — Surface distinct error variants for missing source slot, insufficient stock, and rejected `on_commit`. Test: each path produces its own error. [8a, 8e]
...
- [x] **10d** — Add CLI smoke test asserting `fgk tick` is interruption-safe (Ctrl-C during a callback leaves `last_run_at` unchanged for that task). [10a, 9d]
...
- [x] **11d** — Add an opt-in `run_due_ticks_on_login` runtime hook that callers may enable. Test: when enabled, login path invokes ticks; when disabled, it does not. [11c]
...
- [x] **12e** — Add `docs/spec-v4-overview.md` cross-referencing v1–v3 primitives and showing how v4 composes with them (e.g., place-owned inventory + tick task = restocking trading post). [12a, 12b, 12c, 12d]
...
- [ ] **13b** — Make `fgk new <tmp>` scaffolds pass `cargo test` in-repo without requiring a published `foglet_game` crate so Completion condition 11 is verifiable in CI/dev loops. [13a]
- [ ] **13c** — Re-run completion commands 1–17 after 13b, then check Task 13 and Acceptance criteria gates together if all pass. [13b]
