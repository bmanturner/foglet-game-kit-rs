...
- [ ] **2c** — Parse `[[leaderboards]]` config with `name` and `sort`. Tests: duplicate leaderboard names are rejected. [2a]
...
- [ ] **3a** — Create `crates/foglet_game/src/world_db.rs` with `WorldDb` type and module docs. Test: opens SQLite under a temp dir. [Task 1]
- [ ] **3b** — Create parent directories before opening the DB. Test: nested `world/world.sqlite` path is created. [3a]
...
- [ ] **4a** — Add `world_migrations` table creation. Test: table exists after bootstrap. [3a]
- [ ] **4b** — Add `WorldMigration { version, name, sql }` and apply one migration. Test: version is recorded. [4a]
- [ ] **4c** — Make migration application idempotent. Test: applying the same migration twice records one row and leaves schema valid. [4b]
...
- [ ] **5a** — Add `players` migration with `foglet_user_id`, `handle`, `role`, `security_level`, `first_seen_at`, `last_seen_at`, and `local_dev_key`. [4b]
- [ ] **5b** — Implement `PlayerRecord` and `WorldDb::upsert_player(&FogletContext)`. Test: user_id creates stable record. [5a]
- [ ] **5c** — Support local-dev player keys when `FogletContext.user_id` is missing. Test: two local-dev handles do not collide. [5b]
...
- [ ] **6a** — Add `turn_ledger` migration keyed by player and local date. [5a]
- [ ] **6b** — Add injectable date provider abstraction for turn tests. [6a]
- [ ] **6c** — Implement initial daily allowance creation. Test: new player gets configured allowance. [6b]
...
- [ ] **7a** — Add `world_events` migration with timestamp, kind, player id, message, and metadata JSON. [5a]
...
- [ ] **8a** — Add `leaderboard_scores` migration with named board, player id, score, updated_at. [5a]
- [ ] **8b** — Implement `set_score`. Test: first write creates a row. [8a]
- [ ] **8c** — Implement `increment_score`. Test: increments existing score and creates missing score. [8b]
...
- [ ] **9a** — Expose `WorldDb::transaction` wrapper. Test: commit persists writes. [3a]
- [ ] **9b** — Test rollback on closure error. [9a]
